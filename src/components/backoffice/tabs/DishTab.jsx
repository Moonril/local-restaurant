    ...
    return (
        <section className="p-3 md:p-10">
            ...
            {dishes.map((dish) => (
                ...
                <div key={dish.id} className="border-2 border-slate-500 py-5 rounded-2xl text-center text-2x md:w-xs">
                    <h4 className="text-3xl capitalize mb-2">{dish.name}</h4>
                    <h5 className="font-semibold mb-2 text-2xl">{dish.title}</h5>
                    <img src={dish.image} alt={dish.title} className=" md:w-sm md:h-48 object-cover" />
